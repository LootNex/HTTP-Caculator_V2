...
func AgentRun() {
	for {
		...
		solvedtask.Operation_time = time.Now().Sub(start)
		json_solved_task, err := json.Marshal(solvedtask)
		if err != nil {
			log.Print("wrong json")
		}

		_, err = http.Post("http://localhost:8080/internal/task", "application/json", bytes.NewBufferString(string(json_solved_task)))
		if err != nil {
